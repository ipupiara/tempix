...
static void MX_GPIO_Init(void)
{
  ...
  /* EXTI interrupt init*/
  BSP_IntVectSet (clutchBreak_EXTI_IRQn,0,CPU_INT_KA,EXTI1_IRQHandler);
  BSP_IntEnable(clutchBreak_EXTI_IRQn);
  __HAL_GPIO_EXTI_ENABLE_IT(clutchBreak_Pin);

  BSP_IntVectSet (set_acc_EXTI_IRQn,0,CPU_INT_KA,EXTI2_IRQHandler);
  BSP_IntEnable(set_acc_EXTI_IRQn);
  __HAL_GPIO_EXTI_ENABLE_IT(set_acc_Pin);

  BSP_IntVectSet (res_dec_EXTI_IRQn,0,CPU_INT_KA,EXTI3_IRQHandler);
  BSP_IntEnable(res_dec_EXTI_IRQn);
  __HAL_GPIO_EXTI_ENABLE_IT(res_dec_Pin);

  BSP_IntVectSet (on_off_EXTI_IRQn,0,CPU_INT_KA,EXTI4_IRQHandler);
  BSP_IntEnable(on_off_EXTI_IRQn);
  __HAL_GPIO_EXTI_ENABLE_IT(on_off_Pin);

  BSP_IntVectSet (speedometer_EXTI_IRQn,0,CPU_INT_KA,EXTI9_5_IRQHandler);
...
static void MX_TIM5_Init(void)
{
  ...
  BSP_IntVectSet (TIM5_IRQn,0,CPU_INT_KA,TIM5_IRQHandler);
